	...
	for(int n = 0; n<dataLength;){
		if(JudgeSystem_rxBuff[n] == JUDGESYSTEM_FRAMEHEADER){
			...
			switch (JudgeSystem_rxBuff[n + 5] | (JudgeSystem_rxBuff[n + 6] << 8)){
				...
				case JUDGESYS_CMDID_GAMESTATUS:
					if (Verify_CRC16_Check_Sum(JudgeSystem_rxBuff + n, JUDGESYS_FULLDATALENGTH_GAMESTATUS)){
						memcpy(JudgeSYS.GameStatus.data.dataBuff, &JudgeSystem_rxBuff[n + 7], 
				...
				case JUDGESYS_CMDID_GAMERESULT:
					if (Verify_CRC16_Check_Sum(JudgeSystem_rxBuff + n, JUDGESYS_FULLDATALENGTH_GAMERESULT)){
						memcpy(&JudgeSYS.GameResult.winner, &JudgeSystem_rxBuff[n + 7], 
        ...
        case JUDGESYS_CMDID_ROBOTLIVEINFO:
					if (Verify_CRC16_Check_Sum(JudgeSystem_rxBuff + n, JUDGESYS_FULLDATALENGTH_ROBOTLIVEINFO)){
						memcpy(&JudgeSYS.RobotLiveInfo.data, &JudgeSystem_rxBuff[n + 7], 
        ...
        case JUDGESYS_CMDID_AREAEVENT:
					if (Verify_CRC16_Check_Sum(JudgeSystem_rxBuff + n, JUDGESYS_FULLDATALENGTH_AREAEVENT)){
						memcpy(&JudgeSYS.AeraEvent.data, &JudgeSystem_rxBuff[n + 7], 
        ...
        case JUDGESYS_CMDID_SUPPLYACTION:
					if (Verify_CRC16_Check_Sum(JudgeSystem_rxBuff + n, JUDGESYS_FULLDATALENGTH_SUPPLYACTION)){
						memcpy(&JudgeSYS.SupplyAction.data, &JudgeSystem_rxBuff[n + 7], 
        ...
        case JUDGESYS_CMDID_SUPPLYREQ:
					if (Verify_CRC16_Check_Sum(JudgeSystem_rxBuff + n, JUDGESYS_FULLDATALENGTH_SUPPLYREQ)){
						memcpy(&JudgeSYS.SupplyReq.data, &JudgeSystem_rxBuff[n + 7], 
        ...
        case JUDGESYS_CMDID_ROBOTSTATUS:
					if (Verify_CRC16_Check_Sum(JudgeSystem_rxBuff + n, JUDGESYS_FULLDATALENGTH_ROBOTSTATUS)){
						memcpy(&JudgeSYS.RobotStatus.data, &JudgeSystem_rxBuff[n + 7], 
        ...
        case JUDGESYS_CMDID_POWERHEAT:
					if (Verify_CRC16_Check_Sum(JudgeSystem_rxBuff + n, JUDGESYS_FULLDATALENGTH_POWERHEAT)){
						memcpy(&JudgeSYS.PowerHeatData.data, &JudgeSystem_rxBuff[n + 7], 
        ...
        case JUDGESYS_CMDID_ROBOTPOS:
					if (Verify_CRC16_Check_Sum(JudgeSystem_rxBuff + n, JUDGESYS_FULLDATALENGTH_ROBOTPOS)){
						memcpy(&JudgeSYS.RobotPos.data, &JudgeSystem_rxBuff[n + 7], 
        ...
        case JUDGESYS_CMDID_BUFF:
					if (Verify_CRC16_Check_Sum(JudgeSystem_rxBuff + n, JUDGESYS_FULLDATALENGTH_BUFF)){
						memcpy(&JudgeSYS.RobotBuff.data, &JudgeSystem_rxBuff[n + 7], 
        ...
        case JUDGESYS_CMDID_AERIALENERGY:
					if (Verify_CRC16_Check_Sum(JudgeSystem_rxBuff + n, JUDGESYS_FULLDATALENGTH_AERIALENERGY)){
						memcpy(&JudgeSYS.AerialEnergy.data, &JudgeSystem_rxBuff[n + 7], 
        ...
        case JUDGESYS_CMDID_HURTINFO:
					if (Verify_CRC16_Check_Sum(JudgeSystem_rxBuff + n, JUDGESYS_FULLDATALENGTH_HURTINFO)){
						memcpy(&JudgeSYS.HurtInfo.data, &JudgeSystem_rxBuff[n + 7], 
        ...
        case JUDGESYS_CMDID_SHOOTINFO:
					if (Verify_CRC16_Check_Sum(JudgeSystem_rxBuff + n, JUDGESYS_FULLDATALENGTH_SHOOTINFO)){
						memcpy(&JudgeSYS.ShootInfo.data, &JudgeSystem_rxBuff[n + 7], 
...
/**
  ...
  * @retval None
  */
void Judge_getRobotCommInfo(void){
...
/**
  ...
  * @retval None
  */
void Judge_sendDataToClient(float data1, float data2, float data3, uint8_t mask){
	...
	sendbuff[1] = JUDGESYS_DATALENGTH_CLIENTUPLOADINFO & 0xFF;	/* 数据段长度 低位 19 */
  ...
  JudgeSYS.ClientUploadInfo.data.receiver_ID = JudgeSYS.RobotStatus.data.robot_id + 0x100;
  JudgeSYS.ClientUploadInfo.data.data1 = data1;
...
/**
  ...
  * @retval None
  */
void Judge_sendDataToRobot(uint16_t robotID, uint16_t dataID, uint8_t *data, uint8_t dataLenth){
  uint16_t buffLen = dataLenth + 15;/* FrameHeader(9) + DataHeader(6) */
     ...
     dataID > JUDGESYS_DATAID_ROBOTCOM_MAX)
  {
	...
	/*--FrameHeader_Begin--*/
	sendbuff[0] = 0xA5;			                                    /* 帧起始字节*/
	sendbuff[1] = (dataLenth + 6) & 0xFF;	                          /* 数据段长度 低位 */
	sendbuff[2] = 0;		                            /* 数据段长度 高位 */
	sendbuff[3] = JudgeSYS.sendDataCounter;		                  /* 包序号 */
	sendbuff[4] =  Get_CRC8_Check_Sum(sendbuff, 4, CRC8_INIT);  /* 帧头 CRC8 校验 */
	/*--FrameHeader_End--*/
  ...
  JudgeSYS.RobotComInfo_TX.infoUpdateFlag = 1;
	//发送
